...
- `std.io`: file helpers + print utilities, plus a `Sink` stream (`Sink.Stdout`/`Stderr`/`Stdin`/`File(File)`) with `write`/`try_write`/`flush` and `write_to`/`writeln_to` for writing, `read_line_max`/`read_bytes` for reading, and a `RotatingSink` (size-capped, generation-rolling file logger). `stderr`/`stdin` are reachable via the `std.c.io` stream accessors (`stdout_stream`/`stderr_stream`/`stdin_stream`).
