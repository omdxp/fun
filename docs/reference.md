...
- `std.channel`: bounded blocking channels (ring buffer) with timeout send/recv, channel-level and per-call select wait-slice tuning, adaptive select wait backoff, and select_recv_with/select_recv3_rr_with helpers, built on `std.sync`
