...
- **std/channel.fn**: Bounded blocking ring-buffer channels with timeout send/recv, channel-level and per-call select wait-slice tuning, configurable adaptive select wait backoff, plus select_recv_with and select_recv3_rr_with helpers on top of std.sync.
